...
var GreetingMessage = AbstractAction.extend({
    ...
    init: function(parent, action) {
        ...
        if(!action.attendance) {
            ...
            this.getSession().user_has_group('ma_base_attendance.group_hr_attendance_manager').then(function(has_group) {
                if(has_group) {
                    self.next_action = 'ma_base_attendance.hr_attendance_action_kiosk_mode';
                } else {
                    self.next_action = 'ma_base_attendance.hr_attendance_action_my_attendances';
                }
            });
            return;
        }

        this.next_action = action.next_action || 'ma_base_attendance.hr_attendance_action_my_attendances';
        // no listening to barcode scans if we aren't coming from the kiosk mode (and thus not going back to it with next_action)
        if (this.next_action !== 'ma_base_attendance.hr_attendance_action_kiosk_mode' && this.next_action.tag !== 'hr_attendance_kiosk_mode') {
            this.activeBarcode = false;
        }
...
core.action_registry.add('ma_base_attendance_greeting_message', GreetingMessage);
