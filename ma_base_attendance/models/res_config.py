...
class BaseConfigSettings(models.TransientModel):
    ...
    group_attendance_use_pin = fields.Selection([(0, 'Partners do not need to enter their PIN to check in manually in the "Kiosk Mode".'),
                                                ...
                                                implied_group="ma_base_attendance.group_hr_attendance_use_pin")
    ...
    def set_values(self):
        ...
        for record in self:
            config_parameters.set_param("ma_base_attendance.shift_autocheckout",
                                        record.shift_autocheckout or '0')
            config_parameters.set_param("ma_base_attendance.hex_scanner_is_used", record.hex_scanner_is_used)
    ...
    def get_values(self):
        ...
        config_parameters = self.env["ir.config_parameter"].sudo()
        res.update(
            shift_autocheckout=int(config_parameters.get_param("ma_base_attendance.shift_autocheckout", default=0)),
            hex_scanner_is_used=config_parameters.get_param("ma_base_attendance.hex_scanner_is_used", default=False),
        )
        return res

    @api.model
    def checkout_shifts(self):
        cron_record = self.env.ref('ma_base_attendance.base_attendance_autocheckout')
